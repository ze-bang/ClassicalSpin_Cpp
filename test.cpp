...
void parallel_tempering_pyrochlore(double T_start, double T_end, double Jxx, double Jyy, double Jzz, double gxx, double gyy, double gzz, double h, array<double, 3> field_dir, string dir, const vector<int> &rank_to_write, bool MD){
    filesystem::create_directory(dir);
    Pyrochlore<3> atoms;
    ...
    if (MD){
        MC.molecular_dynamics(temps[0], 1e-3, 10000, 0, 0, 1000, 1e-1, dir);
    }
}
...
int main(int argc, char** argv) {
    ...
    bool MD = argv[12] ? atoi(argv[12]) : 0;
    std::cout << "Initializing parallel tempering calculation with parameters: " << "T = " << 1e-3 << "-" << 10 << " Jpm: " << Jpm << " Jpmpm: " << Jpmpm << " H: " << h << " field direction : " << dir_string << " saving to: " << dir_name << endl;
    parallel_tempering_pyrochlore(1e-3, 10, -2*Jpm - 2*Jpmpm, 1, -2*Jpm + 2*Jpmpm, 0, 0, 1, h, field_dir, sub_dir, {0}, MD);
