...
The benchmark in this code (Table~\ref{tab:bench}) measures
$1.4 \times 10^{7}$ updates/sec/core for the same kernel.  We are
$\sim$\,15$\times$ off peak.  Where does that gap come from?  And which
gaps can we close without rewriting in CUDA?  The next sections answer
that question, ingredient by ingredient.

\begin{table}[t]
\centering
\caption{Measured single-core throughput on a 32-thread Linux box (gcc
13.3, \texttt{-O3 -march=native -flto}).  ``updates/sec'' is the number
of \emph{spin updates} per second; for cluster algorithms it counts
``sweeps $\times$ N sites'' which conflates wall time with sample
quality.}
\label{tab:bench}
\begin{tabular}{lllrrrr}
\toprule
Model & Algorithm & $L$ & $N$ & wall (s) & sweeps/s & updates/s \\
\midrule
Heisenberg honeycomb & Metropolis     & 32 & 2048  & 2.94 & $6.8 \times 10^{3}$ & $1.4 \times 10^{7}$ \\
Heisenberg honeycomb & Overrelaxation & 32 & 2048  & 1.25 & $1.6 \times 10^{4}$ & $3.3 \times 10^{7}$ \\
Heisenberg honeycomb & Wolff          & 32 & 2048  & 0.016 & $3.2 \times 10^{5}$ & $6.5 \times 10^{8}$ \\
Heisenberg honeycomb & Swendsen--Wang & 32 & 2048  & 0.44  & $1.1 \times 10^{4}$ & $2.3 \times 10^{7}$ \\
Kitaev honeycomb     & Metropolis     & 32 & 2048  & 3.02 & $6.6 \times 10^{3}$ & $1.4 \times 10^{7}$ \\
Pyrochlore           & Metropolis     & 6  & 864   & 0.73 & $1.4 \times 10^{4}$ & $1.2 \times 10^{7}$ \\
\midrule
\multicolumn{7}{l}{\emph{Cache-hierarchy effect, Heisenberg honeycomb metropolis:}} \\
Heisenberg honeycomb & Metropolis     & 32  & 2048  & --- & --- & $1.4 \times 10^{7}$ \\
Heisenberg honeycomb & Metropolis     & 64  & 8192  & --- & --- & $1.0 \times 10^{7}$ \\
Heisenberg honeycomb & Metropolis     & 128 & 32768 & --- & --- & $4.7 \times 10^{6}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Why this matters even more for the interaction tables}
The same logic applies to the exchange matrices.  The naive layout
\begin{lstlisting}
std::vector<std::vector<Eigen::Matrix3d>> bilinear_interaction;
\end{lstlisting}
hides three pointer indirections per neighbour: vector
$\to$ inner vector $\to$ Eigen buffer.  Modern atomistic spin codes
(Vampire, UppASD) instead pack the entire $N \times z \times 9$ table
into a single \texttt{double[]} (a ``Structure of Arrays''):
\begin{lstlisting}
double* J_flat;     // size = N * z * 9
\end{lstlisting}
This is the single biggest optimization remaining for our code; it would
likely give another $1.5$--$3\times$ on the Metropolis kernel.
...
% =================================================================
\section{Ingredient III: sweep order and branch behaviour}
% =================================================================
...
% =================================================================
\section{Ingredient IV: better algorithms beat better implementations}
% =================================================================
...
% =================================================================
\section{Ingredient V: parallelism and where to apply it}
% =================================================================
...
% =================================================================
\section{Ingredient VI: reproducibility is a feature, not a luxury}
% =================================================================
...
\texttt{ClassicalSpin\_Cpp} now ticks all thirteen.  The single
remaining gap before head-to-head benchmarking against Vampire/UppASD
on Heisenberg systems is item~3 (a Structure-of-Arrays refactor of the
bilinear table), which is estimated to give a further $1.5$--$3 \times$
on the Metropolis kernel at $L \gtrsim 32$.
...
$\sim 1.4 \times 10^{7}$ Metropolis spin-updates/sec/core for a
short-range Heisenberg model on a single x86 core.  For comparison,
...
sizes.  We are within a factor of $\sim 2$--$5$ of those codes
\emph{without} hand-rolled SIMD intrinsics or a SoA refactor of the
exchange table.  With those two additional ingredients the gap should
close entirely.
...
measured throughput drops from $1.4 \times 10^{7}$ to $4.7 \times 10^{6}$
across this range---a $3 \times$ slowdown that is essentially \emph{all
memory bandwidth}.  This is the regime where SIMD and SoA pay the most.
