...
# filepath: /home/pc_linux/ClassicalSpin_Cpp/util/single_q_fixed_q.py
...
class SingleQFixedQ:
    ...
    with a fixed Q-vector to determine ground state spin configuration and energy.
    """
    # Define parameter bounds (excluding Q1, Q2 which are fixed)
    eta_small = 10**-9
    ...
    def __init__(self, Q1, Q2, L=4, J=[-6.54, 0.15, -3.76, 0.36, -0.21, 1.70, 0.03], B_field=np.array([0, 0, 0])):
        """
        Initialize the single-Q model with fixed Q-vector
        
        Args:
            Q1: First component of the Q-vector
            Q2: Second component of the Q-vector
    ...
    def E_per_UC(self, params):
        """Calculate the energy per unit cell for the single-Q ansatz with fixed Q"""
        alpha_A, alpha_B, phi_A, theta_A, psi_A, phi_B, theta_B, psi_B = params
        Qvec = self.Q1 * self.b1 + self.Q2 * self.b2
    ...
    def find_minimum_energy(self, N_ITERATIONS=10, tol_first_opt=10**-8, tol_second_opt=10**-10):
        """Find the optimal parameters that minimize the energy with fixed Q"""
        opt_params = [0, 0, 0, 0, 0, 0, 0, 0]
        opt_energy = 10**10
        
        for i in range(N_ITERATIONS):
            # Random initial guess (except for Q1, Q2 which are fixed)
            ...
            initial_guess = [alpha_A_guess, alpha_B_guess, 
    ...
    def classify_phase(self, tol=10**-6):
        ...
        # For fixed Q, we can determine the phase directly from Q1, Q2
        if np.abs(self.Q1) < tol and np.abs(self.Q2) < tol:
            # Further determine FM vs AFM from spin configuration
            alpha_A, alpha_B, phi_A, theta_A, psi_A, phi_B, theta_B, psi_B = self.opt_params
            exA, eyA, ezA = self.RotatedBasis(phi_A, theta_A, psi_A)
            exB, eyB, ezB = self.RotatedBasis(phi_B, theta_B, psi_B)
            spin_a_0 = ezA * alpha_A + np.sqrt(1 - alpha_A**2) * exA
            spin_b_0 = ezB * alpha_B + np.sqrt(1 - alpha_B**2) * exB
        ...
        # M point order
        elif (np.abs(self.Q1-0.5) < tol and np.abs(self.Q2) < tol) or (np.abs(self.Q2-0.5) < tol and np.abs(self.Q1) < tol):
            return "Zigzag/Stripy"
        
        # K point order
        elif np.abs(self.Q1-1/3) < tol and np.abs(self.Q2-1/3) < tol:
            return "120° order"
        
        # Incommensurate order
        else:
            return f"Incommensurate order (Q1={self.Q1:.4f}, Q2={self.Q2:.4f})"
    
    def generate_spin_configuration(self):
        """Generate the spin configuration from the optimal parameters"""
        alpha_A, alpha_B, phi_A, theta_A, psi_A, phi_B, theta_B, psi_B = self.opt_params
        Q_vec = self.Q1 * self.b1 + self.Q2 * self.b2
        ...
        for i in range(N):
            ...
            if i % 2 == 0:  # A sublattice
                spin = ezA * alpha_A + np.sqrt(1 - alpha_A**2) * (exA * np.cos(Q_vec.dot(pos)) + eyA * np.sin(Q_vec.dot(pos)))
            else:  # B sublattice
                spin = ezB * alpha_B + np.sqrt(1 - alpha_B**2) * (exB * np.cos(Q_vec.dot(pos)) + eyB * np.sin(Q_vec.dot(pos)))
...
if __name__ == "__main__":
    ...
    # Fixed Q-vector (e.g., for zigzag order)
    Q1, Q2 = 0.5, 0.0
    
    # Create fixed single-Q model
    model = SingleQFixedQ(Q1, Q2, L, J)
    
    # Print results
    print(f"Fixed Q-vector: ({Q1:.4f}, {Q2:.4f})")
