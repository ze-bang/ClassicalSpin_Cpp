...
emitted field, so each analyser setting reads one Cartesian component
of the total moment:
\begin{center}\small
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{llll}
...
A same & $m_x$ & $F_x$ (carries $\qafm$) &
  CEF $\lambda^{6}$ ($E_{23}$), $\lambda^{4}$ ($E_{13}$), and
  $d_z^{\rm eff}\lambda^2$ ($E_{12}$)\\
B cross & $m_x$ & $F_x$ (carries $\qafm$) & $\equiv0$ (closure)\\
B same & $m_z$ & $F_z$ (carries $\qfm$) &
  $\mu\lambda^2+\beta\lambda^1\lambda^2$, $50\times$ the Fe term\\
\bottomrule
\end{tabular}
\end{center}
The $d_z^{\rm eff}\lambda^2$ entry in the same-polarised row is
...
\item \textbf{Geometry-B same-polarised.}  Predicted to be a single
  peak at $(E_{12},2E_{12})$; its strength measures $\beta$, and a map
  led instead by $(\qfm,\qafm)$ would put $\beta$ far below the value
  inferred in geometry A.
