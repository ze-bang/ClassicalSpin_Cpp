...
\begin{figure}[h]
\centering
\includegraphics[width=0.62\textwidth]{tmfeo3_2dcs_final/figures/pulsewidth_verification.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{tmfeo3_2dcs_final/figures/gamma_sharpening.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{tmfeo3_2dcs_final/figures/master_spectra.png}
...
\subsection{Final four-channel verification and the emulated spectra}
\label{sec:finalverify}

The closing check verifies, channel by channel, that the required peaks are
the only (or overwhelmingly dominant) features of the master model
(\cref{fig:emulated}; folded $|\omega_\tau|$ magnitude display, the
experimental convention---which also places both geometry-3 transfer peaks on
the \emph{same} $|\omega_\tau|=E_{12}$ row; in signed frequency the two sit in
opposite quadrants at weak drive, $54{:}1$ for $\lambda^4$ vs $4.2{:}1$ for
$\lambda^6$, the latter softened by the same-side amplitude that the static
$W^{xz}_1$ hybridization contributes):
\begin{itemize}
\item \textbf{$\mathcal G_A$ cross-pol} ($S_z+\lambda^{1,2}$):
  $(\omega_q,E_{12})=1.00$ in \emph{both} $\lambda^1$ and $\lambda^2$; all
  else $\le0.12$; Fe $S_z$ machine-dark ($10^{-5}$).
\item \textbf{$\mathcal G_A$ same-pol} ($S_x+\lambda^{4\ldots7}$):
  $(E_{12},E_{13})=1.00$ in $\lambda^{4,5}$; $(E_{12},E_{23})=1.00$ in
  $\lambda^{6,7}$ with the $W^{xz}_1$-fed cascade at $0.75$ (the irreducible
  companion of the post-pulse buildup); Fe $S_x$ carries the reverse-$W$ echo
  and magnon diagonal at $\omega_t=0.9$, spectrally separated from the CEF
  detections.
\item \textbf{$\mathcal G_B$ cross-pol} ($S_x+\lambda^{4\ldots7}$):
  $(\omega_F,\omega_q)=1.00$ and $(E_{12},\omega_q)=0.62$;
  $\lambda^{4\ldots7}$ \emph{exactly} dark.
\item \textbf{$\mathcal G_B$ same-pol} (prediction; $S_z+\lambda^{1,2}$):
  $(\omega_F,E_{12})=1.00$, $(\omega_q,E_{12})=0.31$, $(E_{12},E_{12})=0.21$.
\end{itemize}

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{tmfeo3_2dcs_final/figures/emulated_2dcs_spectra.png}
\caption{Emulated detected 2DCS spectra of the master model: coherent
$\mu$-weighted channel sums, folded $|\omega_\tau|$ magnitude.  Circles:
required peaks; squares: predictions (geometry-B same-pol).}
\label{fig:emulated}
\end{figure}

\subsection{Everything that was tried: the exhaustive coupling ledger}
\label{sec:exhaustion}

Every symmetry-conceivable coupling component was tested by pair-run
difference on the corrected recipe.  The catalogue is closed:
\begin{center}
\renewcommand{\arraystretch}{1.25}
\footnotesize
\begin{tabular}{llp{0.52\textwidth}}
\toprule
component(s) & verdict & note \\
\midrule
$W^{yy}_1$ & \textbf{alive, adopted} & the geometry-1 peak; displacive mechanism verified \\
$W^{xz}_1$ & \textbf{alive, adopted (0.01)} & static $F_x\lambda^1S_z$ hybridization; post-pulse buildup; assists $(E_{12},\omega_q)$ \\
$\kappa^B_{1y}$ ($B_z$-gated) & \textbf{alive, adopted} & the impulsive $(E_{12},\omega_q)$ generator; gate keeps $\mathcal G_A$ clean \\
$d^{\rm eff}_z$ ($E\!\parallel\!c$) & \textbf{alive, adopted} & exchange-induced E1; sources $\rho_{12}$ for the transfer peaks \\
$W^{xx}_1$ & alive, rejected & floods $F_x$ with a soft-mode comb \\
$W^{xz}_1$ (large) & alive, rejected & $\ge0.05$: back-hybridization leak into $\mathcal G_A$ cross-pol \\
$\kappa^E_{5x}$ & active, negligible & static-$F_x$ dominated; $<10^{-3}$ relative \\
$W^{xy}_1,\ W^{yz}_1$ & exactly inert & $\Gamma_1$-cancelled \\
$W^{zz}_1$ & negligible & $10^{-4}$-level junk \\
$W^{4}$: all six quadratics & exactly inert & $\Delta\le6\times10^{-12}$ \\
$W^{6}_{yy},\ W^{6}_{xz}$ & exactly inert & $\Delta\le7\times10^{-12}$ \\
$\kappa^E_{2y}$ & exactly inert & $\Delta\le3\times10^{-12}$ \\
$\kappa^B$ ($B_x$-gated) & exactly inert & orbit-sign cancellation \\
$K^-$ (all but $K^-_{2y}$) & exactly inert & static scan + dynamics ($10^{-15}$) \\
$K^-_{2y}$ & rejected & reorients $\Gamma_2$ beyond $0.09$\,meV \\
Tm--Tm exchange & rejected a priori & Davydov splitting adds structure; Tm orders at mK \\
\bottomrule
\end{tabular}
\end{center}
Every adopted coupling is therefore \emph{unique within its role}: no static
alternative to $\kappa^B$ exists (the $W^{xz}_1$ window fails both
geometries), no coupling produces same-pol $(\omega_q,E_{12})$, and
$(\omega_q,E_{13})$ arrives only with a stronger $E_{23}$ partner.
