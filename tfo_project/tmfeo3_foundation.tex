...
$(E_{12},q_{\mathrm{AFM}})$ cross peak; see \cref{sec:settled}.
...
because $d_z$ is mirror-odd, while $\bra id_x\ket i$ is allowed.  Since
$m_z$ is likewise mirror-even and $m_x$ mirror-odd, the two analyser
settings are structurally different---one can read level populations and
the other cannot:
\begin{center}\small
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{lll}
...
$(E\!\parallel\!a,H\!\parallel\!c)$ &
  ...
  $+\,w_{E1}(\lambda^1+\lambda^3+\lambda^8)$ ($d_x$, mirror-even)\newline
...
maximum.  The population terms $\lambda^{3,8}$ appear in one
polarisation only; because a population does not oscillate at a
transition frequency they radiate near $\omega_t=0$, making that
channel's contribution rectified rather than resonant.  And $w_{E1}$,
the $E1/M1$ balance, is an $\mathcal O(1)$ number our data do not fix:
...
$0.04$.  The single quantity that responds is the geometry-B rectified
line, which is therefore a measurement of $w_{E1}$ rather than a
nuisance.  Naming these channels by their magnetic component alone---an
``$m_z$ readout''---understates them in two ways: $\mu\lambda^2$ is the
Tm \emph{CEF} moment of the $1\!\leftrightarrow\!2$ transition, not a
magnon, and the electric $d_x$ contributes to the same setting.
...
B cross & $(\qfm,\qafm)=1.00$ & Fe anisotropy/DM sector alone\\
 & $(E_{12},\qafm)=0.45$ & gated vertex $\kappa^B_{1y}$\\
\bottomrule
\end{tabular}
\end{center}
The gated vertex is not interchangeable with a static one: a sweep over
$W^{xz}_1$ shows the static vertex never creates a second resolved
peak, but merges the two delay rows and migrates the leader
($0.38\to0.40\to0.48$\,THz) while leaking hybridisation-shifted
$E_{12}$ emission at $0.44$--$0.45$\,THz.  A mixer \emph{moves} peaks;
a gated converter \emph{adds} them at unshifted lines, and the data
show two resolved peaks at the unshifted energies.
...
\item \textbf{The symmetry-broken coupling.}  Every feature relying on
  $d_z^{\rm eff}\propto\langle F_x\rangle$---including the
  $E_{12}$-excitation family and the $(\qafm,E_{12})$ emission
  peak---must vanish at the spin reorientation, whereas
  conversion-generated features survive as long as the magnon does.
  This is the decisive experiment: a temperature scan through the
  reorientation separates the two mechanisms and measures
  $d_z^{\rm eff}$ by the size of the collapse.
\item \textbf{Geometry-B same-polarised.}  Predicted to be led by
  $(E_{12},2E_{12})$, whose strength measures $\beta$---a map led
  instead by $(\qfm,\qafm)$ would put $\beta$ far below the value
  inferred in geometry A.  Beneath it, at $0.53$, a rectified pair at
  $(\pm E_{12},\omega_t\!\to\!0)$: this is the $\lambda^3$ population
  readout, allowed in this polarisation and no other, and its amplitude
  measures $w_{E1}$ (it falls to $0.09$ with the electric readout off).
  It is also the only optical readout of the $W_3$ population channel
...
(\cref{sec:settled}): one Hamiltonian, with each coupling carrying its own
